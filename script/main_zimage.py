...
# =============================================================
# Z-IMAGE-BASE CONFIG  (tune here, no need to touch the code below)
# =============================================================
COMFY_URL = "http://127.0.0.1:8188"

# Model files (split-file layout, see ComfyUI/models/...)
UNET_NAME = "z_image_bf16.safetensors"        # models/diffusion_models/
CLIP_NAME = "qwen_3_4b.safetensors"           # models/text_encoders/
CLIP_TYPE = "lumina2"                          # Z-Image text encoder type
VAE_NAME  = "ae.safetensors"                   # models/vae/  (Flux VAE)
UPSCALE_MODEL = "RealESRGAN_x4plus.pth"        # models/upscale_models/

# Sampler settings (Z-Image-Base, non-distilled)
STEPS        = 30
CFG          = 6.5                             # sweet spot 6-7; negatives bite harder at higher cfg
SAMPLER_NAME = "euler"
SCHEDULER    = "simple"
DENOISE      = 1.0
SHIFT        = 3.0                             # ModelSamplingAuraFlow shift

# Output resolution (base latent; RealESRGAN upscales x4 afterwards)
WIDTH  = 1536
HEIGHT = 864
# =============================================================
...
# 2. Generate Output Folder (ddmmyyyy_RunX)
...
def build_mega_workflow(prompts):
    # Shared loader nodes (loaded ONCE by ComfyUI for the whole batch)
    workflow = {
        "1": {"class_type": "UNETLoader", "inputs": {"unet_name": UNET_NAME, "weight_dtype": "default"}},
        "2": {"class_type": "CLIPLoader", "inputs": {"clip_name": CLIP_NAME, "type": CLIP_TYPE}},
        "3": {"class_type": "VAELoader", "inputs": {"vae_name": VAE_NAME}},
        "4": {"class_type": "UpscaleModelLoader", "inputs": {"model_name": UPSCALE_MODEL}},
        # Z-Image requires the AuraFlow model-sampling patch (shift) before sampling
        "5": {"class_type": "ModelSamplingAuraFlow", "inputs": {"shift": SHIFT, "model": ["1", 0]}},
    }
    ...
    for i, item in enumerate(prompts):
        ...
        # 16-channel Flux VAE -> must use EmptySD3LatentImage (not EmptyLatentImage)
        workflow[lat_id] = {"class_type": "EmptySD3LatentImage", "inputs": {"width": WIDTH, "height": HEIGHT, "batch_size": 1}}
        ...
        workflow[save_id] = {"class_type": "SaveImage", "inputs": {"filename_prefix": f"ZImage_{idx}", "images": [up_id, 0]}}

    return workflow

def main():
    print(f"Starting Z-Image-Base batch generation...")
    print(f"Output folder created: {OUTPUT_DIR}\n")

    with open(PROMPT_FILE, "r", encoding="utf-8") as f:
        prompts = json.load(f)

    print(f"Building Mega-Graph for {len(prompts)} prompts (steps={STEPS}, cfg={CFG})...")
